...
class FavouritesRepositoryImpl(
    ...
    override fun getFavouritePoems(): Flow<List<Poem>> {
        return poemsDao.getFavouritesFlow().map { entities ->
            entities.map { it.toDomain() }
        }
    }
    ...
    override suspend fun togglePoemFavourite(id: String, isFavourite: Boolean) {
        poemsDao.updateFavourite(id, isFavourite)
    }
}
